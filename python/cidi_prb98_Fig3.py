...
# input data
Nr, Nphi, Nz = 10, 80, 1                     # number of rings and angles
...
for iz in np.arange(1, Nz+1, 1, dtype=int):
    ...
    for ir in np.arange(1, Nr+1, 1, dtype=int):
        ...
        for iphi in np.arange(1, Nphi+1, 1, dtype=int):
            ...
            x = r*np.cos(phi)
            y = r*np.sin(phi)
            if (-0.5*Rext < x < Rext and (x-Rext)/np.sqrt(3) < y < (Rext-x)/np.sqrt(3)) and (x > Rint
                pos3D[k-1,0] = r
                pos3D[k-1,1] = phi
                pos3D[k-1,2] = z
                pos3D[k-1,3] = x
...
#shifted_energy_no_field = [energy for energy in no_E_field_energy]
...
for k in range(2):
    for E in shifted_energy_no_field[0]:
        current_no_field += 2*k*( FD(E + hb*k**2/2/meff, mu/2) - FD(E + hb*k**2/2/meff, -mu/2) )

    for E in shifted_energy_wE_field[0]:
        current_wE_field += 2*k*( FD(E + hb*k**2/2/meff, mu/2) - FD(E + hb*k**2/2/meff, -mu/2) )

    for E in shifted_energy_sE_field[0]:
        current_sE_field += 2*k*( FD(E + hb*k**2/2/meff, mu/2) - FD(E + hb*k**2/2/meff, -mu/2) )


for k in range(2):
    for E in shifted_energy_no_field[0]:
        current_no_field += 2*k*( FD(E + hb*k**2/2/meff, mu) - FD(E + hb*k**2/2/meff, -mu) )

    for E in shifted_energy_wE_field[0]:
        current_wE_field += 2*k*( FD(E + hb*k**2/2/meff, mu) - FD(E + hb*k**2/2/meff, -mu) )

    for E in shifted_energy_sE_field[0]:
        current_sE_field += 2*k*( FD(E + hb*k**2/2/meff, mu) - FD(E + hb*k**2/2/meff, -mu) )
...
if len(sys.argv) > 0:
    if sys.argv[1] == "wsl":
        print("Using WSL...")
        plt.savefig("/mnt/c/Users/Ryan/Desktop/WSL/CiDi/current_plot_tri.png", dpi=150)
else:
    plt.show()
